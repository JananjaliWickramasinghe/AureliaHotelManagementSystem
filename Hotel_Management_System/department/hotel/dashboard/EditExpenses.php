...
if(isset($_POST['editExpenses'])){
	$eid = $_POST['expenses_id'];
	$select = mysqli_query($conn,"select * from expenses where id = '$eid'");
...
if (isset($_POST['submit'])){
	...
	$amount = $_POST['amount'];
		...
		$sql = "UPDATE expenses SET date='$date',section='$section',amount='$amount' where id = '$id'";
	        
	      
			if(mysqli_query($conn,$sql))
			{
				echo "<script> alert('Record Update successfully');</script>";
				echo '<script> location.replace("index.php?tab=manageexpenses");</script>';
			}
			else
			{
...
<div class="contentform">
  ...
  <h2 class="subtitle">Edit Expenses</h2>
